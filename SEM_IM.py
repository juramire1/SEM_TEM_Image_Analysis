...
scale_dict = {80:2.4, 100:1.92, 125:1.42, 200:0.96, 250:0.77, 400:.48, 113:0.9799} # nm /  pixel^(1/2).
# First entry used in Samuel's code.
# last option added by Julian.
# The number on the right side of the ":" is multiplied manually in lines 310 318, and 334
# This conversion is easy to find on the Zeiss, as the nm/pixel ratio is given in the image info.
# If not, use Preview or any image processing program.
# You must change the number based on your magnification used in the SEM image
...
class SEM_IM(object):
    ...
    def crop_bottom(self):
        ...
        try:
            ...
            Cutoff = 768 # 484 for Samuel's images. Size in pixels of the width (y-axis). The bigger the number, the less you cutoff
    ...
    def run_analysis(self):
        ...
        self.labeled = mh.labeled.remove_bordering(self.labeled) # Remove islands at the border
        ...
        for i in range(self.labeled.shape[0]):
            for j in range(self.labeled.shape[1]):
                if self.borders[i,j].all():
                    ...
                    self.borders_int[i,j] += 1 # Creates a matrix of 0 and 1 that signifies the borders
    ...
    def plot_results(self):
        ...
        plt.xticks([])
        plt.yticks([])
        plt.title('%d separate islands' %self.num_islands)
        pylab.imshow(self.labeled_by_area)
        plt.colorbar()
        ...
        # Plot histogram
        plt.subplots_adjust(hspace=0.3) #Gives the plot more space
        plt.subplot(425)
        weights = np.ones_like(self.size_dist) / (len(self.size_dist))
        plt.hist(self.size_dist*(0.9799**2), align = 'mid',weights=weights)
        ...
        # Plot histogram
        plt.subplot(426)
        weights_perim = np.ones_like(self.perimeters[1:-1]) / (len(self.perimeters[1:-1]))
        plt.hist(self.perimeters[1:-1]*0.9799 , align = 'mid',weights=weights_perim)
        ...
        # Plot histogram
        plt.subplot(427)
        weights_perim_d = np.ones_like(self.perimeter_dist) / (len(self.perimeter_dist))
        plt.hist(self.perimeter_dist , align = 'mid',weights=weights_perim_d)
        ...
        plt.subplot(428)
        plt.plot(self.perimeters[1:-1]*0.9799, self.size_dist[1:-1]*0.9799**2, 'o')
        plt.xlabel('Perimeter [nm]')
